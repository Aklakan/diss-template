...
\end{minted}
\caption{Example of an R2RML mapping.}
\label{lst:lst1}
\end{figure}
...
map:eventTitle a d2rq:PropertyBridge ;
    ...
    .
\end{minted}
\caption{D2RQ map for conferences.}
\label{lst:lst2}
\end{figure}
...
\end{minted}
\caption{Example of the Ontop mapping language}
\label{lst:lst3}
\end{figure}
...
object prd:Product
\end{minted}
\caption{Virtuoso RDF views example}
\label{lst:lst4}
\end{figure}
...
\begin{listing}[H]
\caption{EBNF for variable definition expressions}
\label{lst:vardef}
\begin{minted}[fontsize=\scriptsize]{ebnf}
varDefinition = (var '=' rdf-term-ctor-expr)* ;
...
%% === LEFT: SML (plain text — no lexer needed) ===
%\begin{minipage}[t]{0.45\linewidth}
%\centering
%\begin{minted}[fontsize=\tiny,autogobble,bgcolor=lightblue]{text}
%Prefix rdfs: <http://www.w3.org/2000/01/rdf-schema#>
%Prefix xsd: <http://www.w3.org/2001/XMLSchema#>
%Prefix ex: <http://ex.org/>
%
...
%    ?s = uri(?website)
%    ?l = plainLiteral(?name,'en')
%    ?v = typedLiteral(?vacancy,xsd:boolean)
%  Constrain
%    ?s prefix "http://ex.org/"
%  From
%    """SELECT website, name,vacancy FROM hotels"""
%\end{minted}
%\end{minipage}
%&
%% === RIGHT: R2RML (uses global turtle style) ===
%\begin{minipage}[t]{0.52\linewidth}
%\centering
%\begin{minted}[fontsize=\tiny,autogobble]{turtle}
%@prefix rdfs: <http://www.w3.org/2000/01/rdf-schema#> .
%@prefix xsd: <http://www.w3.org/2001/XMLSchema#> .
%@prefix ex: <http://ex.org/> .
%
...
% NOTE: Turtle by now supports prefix - previously it had to be @prefix! - well but minted does not support new variant\ldots
\begin{figure*}[tb]
\centering
%\begin{adjustbox}{width=1.1\textwidth,center}

\begin{tabular}{@{}c@{\hspace{2em}}c@{}}  % increased from 1em to 2em

% === LEFT: SML ===
\begin{minipage}[t]{0.55\linewidth}   % slightly narrower to give more room to right
\centering
\begin{minted}[fontsize=\tiny,autogobble,bgcolor=lightblue,linenos,numbersep=4pt]{text}
Prefix rdfs: <http://www.w3.org/2000/01/rdf-schema#>
Prefix xsd: <http://www.w3.org/2001/XMLSchema#>
Prefix ex: <http://ex.org/>
...
\centering
\begin{minted}[fontsize=\tiny,autogobble,linenos,numbersep=4pt]{turtle}
@prefix rdfs: <http://www.w3.org/2000/01/rdf-schema#> .
@prefix xsd: <http://www.w3.org/2001/XMLSchema#> .
@prefix ex: <http://ex.org/> .
...
%\todo{The rdf:type predicateObjectMap is missing in the `A simple view in SML and R2RML' figure. Maybe use ldots to show that it is left out intentionally}
...
%\centering
%\begin{tabular}{@{}c@{\hspace{1em}}c@{}}
%% === LEFT: SML (plain text, no highlighting needed) ===
...
%\end{minipage}
%&
%% === RIGHT: R2RML (Turtle with rr: namespace) ===
...
% === LEFT: SML ===
\begin{minipage}[t]{0.40\linewidth}   % slightly narrower to give more room to right
\centering
\begin{minted}[fontsize=\tiny,autogobble,bgcolor=lightblue,linenos,numbersep=8pt]{text}
Prefix ex: <http://ex.org/>
...
\centering
\begin{minted}[fontsize=\tiny,autogobble,linenos,numbersep=8pt]{turtle}
@prefix ex: <http://ex.org/> .
