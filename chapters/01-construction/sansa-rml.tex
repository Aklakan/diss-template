...
\begin{figure*}[!ht]
  ...
  \begin{minipage}[t]{0.31\textwidth}
    \centering
    \begin{lstlisting}[language=SPARQL,caption={Original CONSTRUCT query},captionpos=b,frame=single]
CONSTRUCT {
  <ex:s1> <ex:p1> <ex:o1> .
  <ex:s2> <ex:p2> <ex:o2> .
  GRAPH <ex:gn> {
...
WHERE {
  ...
  \begin{minipage}[t]{0.67\textwidth}
    \centering
    \begin{lstlisting}[language=SPARQL,caption={Rewritten using LATERAL},captionpos=b,frame=single]
CONSTRUCT { GRAPH ?g { ?s ?p ?o } }
WHERE {
  SELECT DISTINCT ?g ?s ?p ?o {
    {
  ...
  \vspace*{-5mm}
  \caption{Rewrite of a \texttt{CONSTRUCT} query to its \texttt{SELECT}+\texttt{LATERAL} form (here expressed via \texttt{UNION}+BIND). The identifiers $s_i, p_i, o_i, g_i$ are placeholders for terms; in this example, $g_i$ is either the default graph (\texttt{""}) or a named graph (\texttt{<ex:gn>}). The use of \texttt{DISTINCT} and \texttt{ORDER BY} ensures reproducible, deduplicated output — a challenge for naive \texttt{CONSTRUCT} due to graph merging semantics.}
  \label{fig:construct-to-lateral}
\end{figure*}
