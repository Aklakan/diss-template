...
% NOTE: The lstlistings are intentional because of the mathescape!
%\begin{figure*}[!h]
% \begin{minipage}[t]{0.34\textwidth}
%  \centering
%  \begin{lstlisting}[style=sparql, basicstyle=\ttfamily\tiny, numberstyle=\tiny, mathescape=true]
%# Query $a$
%CONSTRUCT {
...
WHERE {
  SELECT DISTINCT ?s ?p ?o {
      { |$\mathrm{PATTERN}_a$|
        |\textbf{\texttt{LATERAL}}| { { BIND(|$s_{a_i}$|, |$p_{a_i}$|, |$o_{a_i}$| AS
          ?s, ?p, ?o) } UNION ... } } # for i in 1..n
    UNION
      { |$\mathrm{PATTERN}_b$|
        |\textbf{\texttt{LATERAL}}| { { BIND(|$s_{b_j}$|, |$p_{b_j}$|, |$o_{b_j}$| AS
          ?s, ?p, ?o) } UNION ... } } # for j in 1..m
  } ORDER BY ?s ?p ?o
}
  \end{minted}
