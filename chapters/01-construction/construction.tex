...
Knowledge Graph Construction (KGC) is the discipline of transforming raw, heterogeneous data into a structured,
machine-interpretable representation of domain knowledge, organised as a graph of entities and their relationships.
While knowledge graphs can be implemented on various data models—including property graphs,
relational schemas, or document stores—the RDF-based instantiation stands apart:
it grounds the graph in formal logic, enabling reasoning, unambiguous interpretation, and semantic interoperability across systems.

Consequently, RDF knowledge graph construction, when approached comprehensively, is not merely a data engineering task — it is a semantic artefact design process,
requiring explicit attention to ontology alignment, identifier stability, and logical consistency.
\todo{this could be a bridge to data products / product thinking - that the data package with its metadata itself is a unit of value}


% The actual workload automated workload of Knowledge Graph Construction is the data transformation.
In general, there are two primary approaches to transforming data:
\begin{itemize}
  \item \textbf{Materialization:} A processor reads the input datasets and transform them into the output dataset.
  The resulting output is an independent artifact. The phases of reading, transforming, and writing constitute the process known as Extract, Transform, Load (ETL).
  \item \textbf{Virtualization:} The output data is a virtual view over the input data.
  Query rewriting is employed to translate accesses to the virtual dataset into accesses to the input datasets, followed by appropriate post-processing.
\end{itemize}

The two primary approaches for \emph{implementing} data transformation are:
\begin{itemize}
  \item \textbf{Programmatic Approach:} This approach employs a general-purpose programming language to construct the output dataset from input sources.
  It offers maximum flexibility for complex, arbitrary transformations of input records into output structures.
  However, this generality often obscures domain-specific semantics, making the logic harder to reason about, maintain, and optimize compared to specialized tools.

  \item \textbf{Declarative Mapping Approach:} These languages leverage the specific features and structures of the input and output data models, often resulting in more succinct and expressive syntax.
  Crucially, certain declarative mapping languages enable the inversion of the relationship between input and output records.
  This invertibility facilitates \textit{query rewriting}, thereby supporting virtual data access without materializing the output.
  While this eliminates the storage costs and ETL latency associated with material copies, it introduces significant complexity in indexing and optimizing source data access.
  Furthermore, even when theoretically feasible, query rewriting engines may struggle to optimize complex access patterns effectively.
\end{itemize}

%\todo{pragmatically its often data engineering, but for complex use cases it is more}
%This section focuses on this logically grounded instantiation of KGC.
%Knowledge Graph Construction (KGC) is the process of designing and populating a semantically well-founded RDF graph,
%aimed at enabling precise domain representation, interoperability, and reuse.

Its essential facets are:\todo{There was some paper by OEG from 2010 about this - cite!}

\begin{itemize}
\item \textbf{URI design and stable identification} — establishing globally unique, dereferenceable, and persistent IRIs to ensure referential stability and avoid ambiguity across datasets.
\item \textbf{Ontology and schema design} — selecting, adapting, or extending lightweight or heavy-weight ontologies (e.g., schema.org, Wikidata, DBpedia, or domain-specific ones like SNOMED CT or QUDT) to define Classes, Properties, and axioms—prioritising reuse and alignment over ad-hoc modeling.
\item \textbf{Data transformation and mapping} — converting source data into RDF using declarative mapping languages (e.g., RML, D2RQ, or SHACL-based pipelines), while preserving semantics and handling missing/ambiguous values.
\item \textbf{Schema alignment} — harmonising heterogeneous data models through explicit vocabulary mappings (e.g., via \texttt{owl:equivalentClass}, \texttt{skos:exactMatch}), enabling integration without requiring full ontology reconciliation.
\item \textbf{Knowledge base integration (selective interlinking)} — optional but high-impact linking to external authoritative KGs (e.g., Wikidata, DBpedia) where it supports validation, enrichment, or cross-system reasoning— undertaken deliberately and at scale only when justified by use-case needs.
\end{itemize}

%This process typically involves formtransfo heterogeneous
...
In~\cref{sec:kgc-preliminaries}, the foundations of RDF knowledge graph constructions by means of declarative mappings are introduced.
%This section also explains the virtualization and materialization approaches to data integration.
In~\cref{sec:sml}, the \emph{Sparqlification Mapping Language} (SML) is presented.
\Cref{sec:sparklify} present our approach that leverages Apache Spark for generic SPARQL query evaluation using SPARQL-to-SQL rewriting.
%This way, SPARQL can be leveraged for KGC construction.
We also discuss how map-reduce is a bad match for generic SPARQL query evaluation.
\Cref{sec:sansa-rml} presents a revised approach for KGC that extends SPARQL with features that make the language suitable for ETL such
that Apache Spark's map-reduce execution model can be properly leveraged.
In~\cref{sec:related-work} we present relevant approaches.
In~\cref{sec:conclusions} we conclude this chapter.
\todo{revise some formulations}

\section{Mapping Languages for Knowledge Graph Construction}
%\subsection{Virtual vs Materialized Data Integration}

\todo{TODO}
\section{Big Data for Scaling ETL}
\todo{TODO}

\section{OBDA / SPARQL-to-SQL on Big Data}
%\label{sec:intro}
...
\section{Sansa RML Toolkit - SPARQL-based ETL on Apache Spark}
\todo{Essentially: Sparklify query rewriting on Big Data Frameworks possible but the execution model does not (generally) fit the Map-Reduce paradigm - so performance sucks. But we can leverage Apache Spark for ETL based on RML and this works very well.}
{
\let\section\subsection
\let\subsection\subsubsection
\subimport{./}{sansa-rml}
